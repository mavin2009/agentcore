...
- [`./quickstarts/python.md`](./quickstarts/python.md): local build, first graph, metadata, streaming, message state, recorded once-only work, intelligence state, persistent subgraph sessions, and pause/resume
- [`./quickstarts/cpp.md`](./quickstarts/cpp.md): native build, `ExecutionEngine`, state patches, and example executables
- [`./integrations/mcp.md`](./integrations/mcp.md): MCP over `stdio`, including tools, prompts, resources, completions, mirrored tool registration, and exposing AgentCore surfaces to MCP clients
- [`./integrations/opentelemetry.md`](./integrations/opentelemetry.md): opt-in OpenTelemetry tracing and metrics for invoke, stream, and pause/resume flows
...
- [`./migration/langgraph-to-agentcore.md`](./migration/langgraph-to-agentcore.md): shortest-path guide for moving a LangGraph-style `StateGraph` to AgentCore, including schema reducers and message history
...
- If you want to connect AgentCore to an MCP tool ecosystem, start with [`./integrations/mcp.md`](./integrations/mcp.md).
- If you want AgentCore to show up in an existing observability stack, read [`./integrations/opentelemetry.md`](./integrations/opentelemetry.md).
...
- Interop smoke coverage: [`../python/tests/mcp_runtime_smoke.py`](../python/tests/mcp_runtime_smoke.py)
