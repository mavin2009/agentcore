...
StatePatch build_subgraph_input_patch(
    ...
    if (!seed_knowledge_graph || !binding.propagate_knowledge_graph) {
        return patch;
    }

    for (KnowledgeEntityId entity_id = 1U; entity_id <= parent_state.knowledge_graph().entity_count(); ++entity_id) {
        const KnowledgeEntity* entity = parent_state.knowledge_graph().find_entity(entity_id);
        if (entity == nullptr) {
            continue;
        }
        patch.knowledge_graph.entities.push_back(KnowledgeEntityWrite{
            rebase_string_id(parent_state, child_state, entity->label),
            rebase_blob_ref(parent_state, child_state, entity->payload)
        });
    }

    const std::vector<const KnowledgeTriple*> triples =
        parent_state.knowledge_graph().match(std::nullopt, std::nullopt, std::nullopt);
    patch.knowledge_graph.triples.reserve(triples.size());
    for (const KnowledgeTriple* triple : triples) {
        if (triple == nullptr) {
            continue;
        }
        const KnowledgeEntity* subject = parent_state.knowledge_graph().find_entity(triple->subject);
        const KnowledgeEntity* object = parent_state.knowledge_graph().find_entity(triple->object);
        if (subject == nullptr || object == nullptr) {
            continue;
        }
        patch.knowledge_graph.triples.push_back(KnowledgeTripleWrite{
            rebase_string_id(parent_state, child_state, subject->label),
            rebase_string_id(parent_state, child_state, triple->relation),
            rebase_string_id(parent_state, child_state, object->label),
            rebase_blob_ref(parent_state, child_state, triple->payload)
        });
    }
    return patch;
}
